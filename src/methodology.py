...
SIC-prefix → bucket table matched LONGEST-prefix-first — the exact structure already
used for the industry-risk tiers in src/rating.py (`sic_industry_risk` /
`_industry_risk`). Every issuer already carries an EDGAR SIC on `companies.sic`.
...
confidence and record the disagreement. SIC stays the universal primary because TRBC
only covers the LSEG subset.

Financial-sector issuers (SIC 60–67) are gated out of the industrial cash-flow model
today (see `financial_sector_note` in src/rating.py); we still classify them to each
agency's financial-institution bucket and flag that the corporate scorecard doesn't
apply, so teammates route them to the right (separate) framework.
"""
